...
## 1) Subcontinental PCA
# 1a) Function for plotting only pc1 & 2 and save as a pdf 
```{r}
# set working directory 
setwd("~/Desktop/Broad/alicia/HGDP_TGP/hgdp_tgp") 

# read-in gnomAD metadata 
hgdp_tgp <- read.delim('~/Desktop/Broad/alicia/pca_subcont/gnomad_meta_v1.tsv', header=T, sep='\t') %>% select(s, project_meta.title, starts_with('hgdp_tgp_meta')) 
...
# function 
load_pop_make_pca_plot <- function(pop_name, type_of_ds) {
  
  # read in pc scores to plot 
  ref_unrel <- read.table(gzfile(paste0('UPDATED_scores_for_pca_', type_of_ds, '/subcont_pca_', pop_name, '_scores.txt.bgz')), header=T)
  proj_rel <- read.table(gzfile(paste0('UPDATED_scores_for_pca_', type_of_ds, '/subcont_pca_', pop_name, '_projected_scores.txt.bgz')), header=T)
  ...
  pca_subcont <- bind_rows(ref_unrel, proj_rel) %>% left_join(hgdp_tgp)
  
  # color for populations within each region  
  nb.cols <- length(unique(pca_subcont$hgdp_tgp_meta.Population)) # number of subcontinental regions within each region/continent 
  mycolors <- colorRampPalette(brewer.pal(7, "Set1"))(nb.cols) # set color palettes based on that number 
  ...
  p_subcont <- ggplot(pca_subcont, aes(x=PC1, y=PC2, color=hgdp_tgp_meta.Population, shape=project_meta.title, text=s)) +
    ...
    labs(shape='Project', title=paste(pop_name, 'PCA (', type_of_ds, ')'))
  
  ggsave(paste0('plots/PCA/', type_of_ds, '/', pop_name, '_subcontPCA_1n2_', type_of_ds, '.pdf'), p_subcont) # save plot to pdf 
  ggplotly(p_subcont, tooltip='text') # make plot interactive 
}

# list of population names 
pop_names <- c('AFR', 'AMR', 'CSA', 'EAS', 'EUR', 'MID', 'OCE')

# plot subcontinental pcas (only pc1 & 2) for each region 
for (pop in pop_names) {
  load_pop_make_pca_plot(pop, 'preoutlier') 
}
```

# 1b) function to plot subcont PCAs in a grid upto the 8th PC 
```{r}
# plot function 1
...
load_pop_make_pca_plot <- function(pop_name, type_of_ds) {
  ...
  # combine the ref and proj data into one dataset and then join that to the metadata to add additional info for those samples 
  pca_subcont <- bind_rows(ref_unrel, proj_rel) %>% left_join(hgdp_tgp) %>% arrange(hgdp_tgp_meta.Population)
  ...
  ggsave(paste0('plots/PCA/', type_of_ds, '/', pop_name, '_subcontPCA_1to8_', type_of_ds, '.pdf'), p_subcont_legend, width=15, height=4) # save plot to pdf 
...
## figure out how to save each result to a variable and run the next chunk in a consise way 
n = 3
...
# obtain region/continent colors from the metadata for consistency across all plots
reg_colr_df <- unique(pca[c("hgdp_tgp_meta.Genetic.region", "hgdp_tgp_meta.Continent.colors")])
...
p <- ggplot(pca, aes(x=PC1, y=PC2, color=hgdp_tgp_meta.Genetic.region, shape=project_meta.title, text=s)) +
  geom_point() +
  theme_classic() +
  scale_color_manual(values = reg_colr, name='Region')+
  labs(shape="Project", title="UPDATED Global PCA (before removing outliers)")
...
```{r}
sin_pca_plot <- function(pca_global, first_PC, second_PC, bottom=F) {
  #nb.cols <- length(unique(pca_global$hgdp_tgp_meta.Population))
  #mycolors <- colorRampPalette(brewer.pal(7, "Set1"))(nb.cols)
  # obtain region/continent colors from the metadata for consistency across all plots
  reg_colr_df <- unique(pca_global[c("hgdp_tgp_meta.Genetic.region", "hgdp_tgp_meta.Continent.colors")])
  reg_colr <- reg_colr_df %>% pull(hgdp_tgp_meta.Continent.colors, hgdp_tgp_meta.Genetic.region)

  
  is_tgp <- unique(pca_global$hgdp_tgp_meta.Genetic.region) %in% subset(pca_global, hgdp_tgp_meta.Project=='1000 Genomes')$hgdp_tgp_meta.Genetic.region
  myshapes <- ifelse(is_tgp, 16, 4)
  
  p_global <- ggplot(pca_global, aes_string(x=first_PC, y=second_PC, color='hgdp_tgp_meta.Genetic.region', shape='project_meta.title', text='s')) +
    geom_point(size=2) +
    theme_classic() +
    #scale_shape_manual(values=myshapes, labels=unique(pca_subcont$hgdp_tgp_meta.Population), name='Population') +
    scale_color_manual(values=reg_colr, labels=unique(pca_global$hgdp_tgp_meta.Genetic.region), name='Region') +
    labs(shape="Project", title='global') +
    theme(text = element_text(size=14, color='black'),
          legend.title = element_text(size = 12), 
          legend.text = element_text(size = 9),
          legend.key.size = unit(0.7, "lines")) +
...
# combine the ref and proj data into one dataset and then join that to the metadata to add additional info for those samples 
pca_global <- bind_rows(ref_unrel, proj_rel) %>%
  left_join(hgdp_tgp) %>%
  arrange(hgdp_tgp_meta.Genetic.region)
  
p_1_2 <- sin_pca_plot(pca_global, 'PC1', 'PC2')
p_1_2_bottom <- sin_pca_plot(pca_global, 'PC1', 'PC2', TRUE) 
p_3_4 <- sin_pca_plot(pca_global, 'PC3', 'PC4')
p_5_6 <- sin_pca_plot(pca_global, 'PC5', 'PC6')
p_7_8 <- sin_pca_plot(pca_global, 'PC7', 'PC8')
p_global <- plot_grid(p_1_2 + theme(legend.position="none"), p_3_4 + theme(legend.position="none"), 
                         p_5_6 + theme(legend.position="none"), p_7_8 + theme(legend.position="none"), nrow=1)
legend <- get_legend(p_1_2)
p_global_legend <- plot_grid(p_global, legend, rel_widths = c(4, 1))
  
ggsave(paste0('UPDATED_pca_plots_preoutlier/UPDATED_global_pca_1to8.pdf'), p_global_legend, width=15, height=4)
  
#ggplotly(p_global_legend, tooltip='text')
```
...
p <- ggplot(pca, aes(x=PC1, y=PC2, color=hgdp_tgp_meta.Genetic.region, shape=project_meta.title, text=s)) +
  ...
  theme_classic() +
  scale_color_manual(values = reg_colr, name='Region')+
  labs(shape="Project", title="UPDATED Global PCA (after removing outliers)")
...
ggsave(paste0('UPDATED_pca_plots_postoutlier/UPDATED_global_pca_1to8.pdf'), p_global_legend, width=15, height=4)
...
# JUNK CODE
```{r}
# obtain population colors from the metadata for consistency across all plots
pop_colr_df <- unique(pca_subcont[c("hgdp_tgp_meta.Population", "hgdp_tgp_meta.Pop.colors")])
