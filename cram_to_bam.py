...
def cram_to_bam(b: hb.batch.Batch, input_cram: hb.resource.ResourceFile, ref_fasta: hb.resource.ResourceGroup,
                bam_out_name: str = None, memory: int = 15, samtools_image: str = None, out_dir: str = None):
    docker_image = samtools_image if samtools_image else 'gcr.io/genomics-tools/samtools'

    out_bam_name = bam_out_name + '.bam'

    # output_bam_size: float = bytes_to_gb(input_cram) / 0.40
    # ref_size: float = bytes_to_gb(ref_fasta) + bytes_to_gb(ref_index)
    # disk_size: int = round(bytes_to_gb(input_cram) + output_bam_size + ref_size) + 25
    ...
    crams_to_bams.command(f'samtools view -b -T {ref_fasta.fasta} -o {out_bam_name} {input_cram}')
    ...
    b.write_output(crams_to_bams.bamout, f'{out_dir}/Gambian-CRAMS/{out_bam_name}')
    b.write_output(crams_to_bams.bamind, f'{out_dir}/Gambian-CRAMS/{out_bam_name}.bai')
...
if __name__ == '__main__':
    ...
    fasta = crams_to_bams_job.read_input_group(**{'fasta': args.ref_fasta,
    ...
    for cram in cram_files:
        ...
        in_cram = crams_to_bams_job.read_input(cram)
        convert_cram_to_bam = cram_to_bam(b=crams_to_bams_job, input_cram=in_cram, ref_fasta=fasta,
