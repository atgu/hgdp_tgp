---
title: "Updating population labels and geographical coordinates"
...
hgdp_tgp_unfil <- read.delim('~/Desktop/Broad/alicia/HGDP_TGP/hgdp_tgp/gnomad_meta_v1.tsv', header=T, sep='\t') 
...
new_labels <- read.delim('~/Desktop/Broad/alicia/HGDP_TGP/hgdp_wgs.20190516.metadata.txt', header=T, sep='\t') 
...
# write out files
#write.table(final_metadata, '~/Desktop/Broad/alicia/HGDP_TGP/gnomad_meta_updated.tsv', row.names=FALSE, sep="\t", quote = FALSE) # updated metadata - everything merged 
#write.table(plot_metadata, '~/Desktop/Broad/alicia/HGDP_TGP/gnomad_meta_for_plots.tsv', row.names=FALSE, sep="\t", quote = FALSE) # plotting - part of the metadata 
```
...
# read-in PLINK fam file and update the population labels for F_ST analysis
plink.fam <- read.delim('~/Desktop/Broad/alicia/HGDP_TGP/fst_and_f2/hgdp_tgp.fam', header = FALSE, sep = "\t")
updated.fam <- plink.fam %>%left_join(plot_metadata, by = c("V2" ="s")) %>% select(population, V2,V3,V4,V5,V6)
#write.table(updated.fam, '~/Desktop/Broad/alicia/HGDP_TGP/fst_and_f2/hgdp_tgp_updated.fam', row.names=FALSE, col.names = FALSE, sep="\t", quote = FALSE) 
```
